...
def main(activation="relu", base_lr=0.01, batch_size=512, dataset="mnist"):
    ...
    try:
        ...
        Delta_b_2s = []
        for i in range(N_TRIALS):
            Delta_W_2, Delta_b_2 = get_Delta_W2_dict(muPs_1[i], muPs_2[i], normalize_first=True)
        ...
        avg_max_acts_diversity_df.to_csv(os.path.join(figures_dir, 'muP_max_acts_' + version + '.csv'),
                                         header=True, index=True)
